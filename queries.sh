...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams AS t LEFT JOIN games AS gw ON t.team_id = gw.winner_id LEFT JOIN games AS go ON t.team_id = go.opponent_id WHERE (gw.year = 2014 OR go.year = 2014) AND (gw.round = 'Eighth-Final' OR go.round = 'Eighth-Final')")"
